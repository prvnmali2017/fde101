...
<!-- This module is for AWS customers. The headline benefit is data residency via Bedrock. Frame it as the enterprise-unblocking module. -->


---

# When you reach for AWS

- Customer mandate: stay in our AWS/region
- Existing AWS footprint
- Security wants VPC + IAM

<!-- Tie back to discovery (Ch 2) — these signals surface in the technical landscape questions. -->
...
<!-- Walk the architecture diagram. Emphasize nothing leaves the account — Bedrock and OpenSearch are in-region, accessed via VPC endpoints. -->


---

# Compute: ECS Fargate vs App Runner

- App Runner: simplest, push image → URL
- ECS Fargate: full VPC/network control
- EKS: only if customer already runs K8s

<!-- Default to App Runner for a fast pilot, Fargate when security needs VPC control. EKS only if they demand it (your Tabcorp experience applies). -->
...
<!-- ECR is the private registry. Mention image scanning — security teams love it. -->
...
<!-- This is the centerpiece. Show how the swappable LLM design from Ch 4 means only the provider call changes. -->
...
<!-- Remind: change embedding model = re-ingest everything. Match ingest and query embeddings. -->
...
<!-- Direct upgrade from ChromaDB. Same RAG code, different store. Hybrid search helps with order IDs/SKUs. -->


---

# Fastest path: Bedrock Knowledge Bases

- Point at an S3 bucket of docs
- Managed chunk + embed + retrieve
- Less control, fastest pilot

<!-- Offer the trade-off: KB = speed, OpenSearch = control. For a 2-week pilot, KB can be the right call. -->
...
<!-- Contrast with the local `.env`. Production never ships secrets in images. -->
...
<!-- Two layers: the app's permissions (IAM task role) and the users' auth (Cognito/SSO). Your IAM background is the asset. -->
...
<!-- Keep traffic private. VPC endpoints mean Bedrock/OpenSearch calls never traverse the internet — a security-review winner. -->
...
<!-- Reuse the `/health` endpoint and add metrics. SRE habits build customer confidence. -->
...
<!-- IaC is how you redeploy in another account cleanly. Show the reference snippet in the demo. -->
...
<!-- Iterative pilot releases. Tie to Ch 3 — ship small and often. -->
...
<!-- Customers watch pilot spend. Know the cost levers. OpenSearch Serverless has a minimum cost — mention KB or pgvector as cheaper alternatives for tiny pilots. -->
...
<!-- This slide is your security-review answer sheet. Walk it as the customer's checklist. -->


---

# Topic: Multi-Agent Orchestration on AWS
...
<!-- Motivate the jump from single-agent (Ch 4) to multi-agent. As pilots grow, one mega-prompt becomes brittle. Splitting into specialist agents with a router/supervisor keeps each agent simple, testable, and independently improvable. Use the RetailCo split as the running example. -->


---

# AWS services for multi-agent
...
<!-- Recap. Same app from the Core Track, now enterprise-deployed on AWS. -->
